...
class Deconv(nn.Module):
    def __init__(self, in_ch, out_ch, k_s):
        ...
        self.deconv = nn.ConvTranspose2d(
            in_channels=out_ch, out_channels=out_ch, kernel_size=2, stride=2, bias=False)
...
class FCN(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=3,
                               kernel_size=5, padding=2, bias=False)
        self.conv3 = nn.Conv2d(in_channels=3, out_channels=1,
                               kernel_size=3, padding=1, bias=False)
        self.conv3_bias = nn.Conv2d(in_channels=3, out_channels=3,
                                    kernel_size=4, padding=1, stride=2, bias=False)
        self.conv3_bias_2 = nn.Conv2d(in_channels=3, out_channels=1,
                                      kernel_size=3, padding=1, bias=False)
        self.fc1 = nn.Linear(in_features=480*280,
    ...
    def forward(self, x):
        ...
        x_bias = F.relu(self.conv3_bias_2(F.relu(self.conv3_bias(x), inplace=True)), inplace=True)
        x = F.relu(self.conv3(x), inplace=True)
        bias = self.fc2(F.relu(self.fc1(x_bias.view(x_bias.size()[0], -1))))
        # num = self.fc2(F.relu(self.fc1(x.view(x.size()[0], -1))))
        return x, bias


if __name__ == "__main__":
    net = FCN()
    ts = (torch.randn([3, 3, 560, 960]).cuda())
    net.cuda()
    pre_map, bias = net(ts)
